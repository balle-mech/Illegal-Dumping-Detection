...
def ntu_det_postproc(vid, det_results):
    ...
    1. 重複を削除する
    2. 1人の場合は、そのまま返す
    3. 2人の場合は、1人ずつのbboxを返す
    4. 3人以上の場合は、bboxを返す

    ※今回はカスタムデータセットを使うため、この関数は使わない
    """
...
def ntu_pose_extraction(vid, skip_postproc=False):
    """
    動画からポーズ情報を抽出し、姿勢推定を行う
    Args:
        vid (str): 動画のパス
        skip_postproc (bool): 後処理をスキップするかどうか
    """
    tmp_dir = TemporaryDirectory()
    # frame_extractを使って、動画をフレームに分割し、フレーム（画像）をtmp_dirに保存する
