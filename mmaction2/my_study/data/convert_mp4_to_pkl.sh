...
echo "変換元のmp4ファイルが入っているディレクトリ/my_study以降を指定 ex: data/rawvideo_dataset"
read directory_original

# echo "変換後のpklファイルを保存するディレクトリ/my_study以降を指定 ex: data/pkl_dataset"
...
for filepath in $(ls -1 my_study/${directory_original}/${class_num}_${data_type}_*.mp4 | sort -V); do
...
# for filepath in $(ls -1 my_study/${directory_original}/${class_num}_*.mp4 | sort -V); do
#   file_name=$(basename $filepath)
#   # .mp4を削除して、代わりに.pklを追加する
#   new_name=${file_name%.mp4}.pkl

#   echo my_study/${directory_converted}/${new_name}を作成します。
#   python my_study/ntu_pose_extraction.py "$filepath" "my_study/${directory_converted}/${new_name}" --skip-postproc
#   echo my_study/${directory_converted}/${new_name}を作成しました。
# done
